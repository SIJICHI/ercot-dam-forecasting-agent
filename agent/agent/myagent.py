...
def graph_factory(
    ...
    agent_forecaster = create_agent(
        ...
        system_prompt=make_system_prompt(
            ...
            "- get_deployment_info: Retrieve deployment information by deployment_id\n"
            "- get_deployment_features: Retrieve feature information for a deployment by deployment_id\n"
            "- get_model_info_from_deployment: Retrieve model information from a deployment\n"
            "- generate_prediction_data_template: Generate a prediction input data template for the deployment\n"
            "- predict_realtime: Execute real-time prediction using a DataRobot deployment\n"
            "\n"
            "Steps to follow:\n"
            "1. Call get_deployment_info with the deployment_id to confirm the deployment is accessible.\n"
            "2. Call generate_prediction_data_template with the deployment_id and forecast parameters "
            "(forecast_range_start, forecast_range_end) to prepare the prediction input.\n"
            "3. Call predict_realtime with the deployment_id and the generated template "
            "to obtain the 24-hour DAM price predictions.\n"
            "4. Return the raw prediction results (timestamps and predicted DAM prices) to the next agent.\n"
            "\n"
